...
def falling(i):
    ...
    t_long  = ts.utc(ts.now().utc.year, ts.now().utc.month, ts.now().utc.day, ts.now().utc.hour, ts.now().utc.minute+range(5*12*24*2))

    try:
        TIANHE = load.tle_file('https://celestrak.com/satcat/tle.php?CATNR=48274', reload=True)[0]
    except:
        print('cant connect to database')
        try:
            TIANHE = api.load.tle('https://celestrak.com/NORAD/elements/stations.txt')['TIANHE']
        except:
            print('load old data')
            try:
                with open('/home/pi/Desktop/tle.php') as myfile:
                    lines = myfile.read().splitlines()
                    TIANHE = EarthSatellite(lines[1],lines[2],lines[0],ts)
            except:
                pass
            pass
        pass
    ...
    try:
        print('orbital elements epoch:\n'+str(TIANHE.epoch.astimezone(tz)))
        print('{:.2f} hours after epoch'.format((ts.now()-TIANHE.epoch)*24))
    except:
        pass

    ra_0, dec_0, edistance_0 = (TIANHE).at(ts.now()).radec()
    ra_5, dec_5, edistance_5 = (TIANHE).at(ts.utc(ts.now().utc.year, ts.now().utc.month, ts.now().utc.day, ts.now().utc.hour, ts.now().utc.minute+5, ts.now().utc.second)).radec()
    ra_10, dec_10, edistance_10 = (TIANHE).at(ts.utc(ts.now().utc.year, ts.now().utc.month, ts.now().utc.day, ts.now().utc.hour, ts.now().utc.minute+10, ts.now().utc.second)).radec()
    ra_15, dec_15, edistance_15 = (TIANHE).at(ts.utc(ts.now().utc.year, ts.now().utc.month, ts.now().utc.day, ts.now().utc.hour, ts.now().utc.minute+15, ts.now().utc.second)).radec()
    ra, dec, edistance = (TIANHE).at(t_range).radec()
    #alt, az, tdistance = (TIANHE - hokoon).at(t_range).altaz()

    terr_orbit  = wgs84.subpoint(TIANHE.at(t_range))
    to_ground   = wgs84.subpoint(TIANHE.at(t_long))
    subpoint_0  = wgs84.subpoint(TIANHE.at(ts.now()))
    subpoint_5  = wgs84.subpoint(TIANHE.at(ts.utc(ts.now().utc.year, ts.now().utc.month, ts.now().utc.day, ts.now().utc.hour, ts.now().utc.minute+5, ts.now().utc.second)))
    subpoint_10 = wgs84.subpoint(TIANHE.at(ts.utc(ts.now().utc.year, ts.now().utc.month, ts.now().utc.day, ts.now().utc.hour, ts.now().utc.minute+10, ts.now().utc.second)))
    subpoint_15 = wgs84.subpoint(TIANHE.at(ts.utc(ts.now().utc.year, ts.now().utc.month, ts.now().utc.day, ts.now().utc.hour, ts.now().utc.minute+15, ts.now().utc.second)))
    geolocator  = Nominatim(user_agent="geoapiExercises")
    overhead    = geolocator.reverse(str(subpoint_0.latitude.degrees)+","+str(subpoint_0.longitude.degrees))

    try:
        print('TIANHE is now '+str(int(subpoint_0.elevation.km))+'km over '+str(overhead.raw['address'].get('country', '')))
    except:
        print('TIANHE is now '+str(int(subpoint_0.elevation.km))+'km over Middle of NoWhere')
...
##        pass
    ...
    ax0.add_collection(LineCollection(lines_xy, colors=(0.5,0.5,0.5,0.5), linewidths=1, zorder=1, transform=ccrs.Geodetic()))
    ax0.scatter(stars['ra_degrees'][bright_stars], stars['dec_degrees'][bright_stars], s=marker_size, color='k', zorder=2, transform=ccrs.PlateCarree())
...
#     ax0.set_ylim(dec_0.degrees-limit*3, dec_0.degrees+limit*3)
    ...
    textstr = ''
    try:
        if str(overhead.raw['address'].get('city', '')) != '':
            textstr = ('TIANHE is now '+str(int(subpoint_0.elevation.km))+'km over '+
                       str(overhead.raw['address'].get('country', ''))+', '+str(overhead.raw['address'].get('city', ''))+'\n'+
                       'LAT: '+str(subpoint_0.latitude.dstr())+', LON: '+str(subpoint_0.longitude.dstr())+'\n'+
                       str(ts.now().astimezone(tz).strftime('%d/%m/%Y %H:%M:%S'))+' HKT'+'\n'+
                       'orbital elements updated: '+'{:.2f} hours ago'.format((ts.now()-TIANHE.epoch)*24))
        else:
            textstr = ('TIANHE is now '+str(int(subpoint_0.elevation.km))+'km over '+
                       str(overhead.raw['address'].get('country', ''))+'\n'+
                       'LAT: '+str(subpoint_0.latitude.dstr())+', LON: '+str(subpoint_0.longitude.dstr())+'\n'+
                       str(ts.now().astimezone(tz).strftime('%d/%m/%Y %H:%M:%S'))+' HKT'+'\n'+
                       'orbital elements updated: '+'{:.2f} hours ago'.format((ts.now()-TIANHE.epoch)*24))
    except:
        textstr = ('TIANHE is now '+str(int(subpoint_0.elevation.km))+'km over Middle of NoWhere'+'\n'+
                   'LAT: '+str(subpoint_0.latitude.dstr())+', LON: '+str(subpoint_0.longitude.dstr())+'\n'+
                   str(ts.now().astimezone(tz).strftime('%d/%m/%Y %H:%M:%S'))+' HKT'+'\n'+
                   'orbital elements updated: '+'{:.2f} hours ago'.format((ts.now()-TIANHE.epoch)*24))
    ...
    # global plot
    ax1 = plt.subplot(3,1,2,projection=ccrs.Orthographic(central_longitude=subpoint_0.longitude.degrees, central_latitude=subpoint_0.latitude.degrees))
    ax1.coastlines()
    ax1.set_global()
    ...
    if subpoint_5.latitude.degrees >= subpoint_0.latitude.degrees:
        ax1.annotate(' now ', xy=(subpoint_0.longitude.degrees,subpoint_0.latitude.degrees),  xycoords=ccrs.PlateCarree()._as_mpl_transform(ax1), ha='left', va='top')
        ax1.annotate(' T+5m ', xy=(subpoint_5.longitude.degrees,subpoint_5.latitude.degrees),  xycoords=ccrs.PlateCarree()._as_mpl_transform(ax1), ha='left', va='top')
        ax1.annotate(' T+10m ', xy=(subpoint_10.longitude.degrees,subpoint_10.latitude.degrees),  xycoords=ccrs.PlateCarree()._as_mpl_transform(ax1), ha='left', va='top')
        ax1.annotate(' T+15m ', xy=(subpoint_15.longitude.degrees,subpoint_15.latitude.degrees),  xycoords=ccrs.PlateCarree()._as_mpl_transform(ax1), ha='left', va='top')
    else:
        ax1.annotate(' now ', xy=(subpoint_0.longitude.degrees,subpoint_0.latitude.degrees),  xycoords=ccrs.PlateCarree()._as_mpl_transform(ax1), ha='right', va='top')
        ax1.annotate(' T+5m ', xy=(subpoint_5.longitude.degrees,subpoint_5.latitude.degrees),  xycoords=ccrs.PlateCarree()._as_mpl_transform(ax1), ha='right', va='top')
        ax1.annotate(' T+10m ', xy=(subpoint_10.longitude.degrees,subpoint_10.latitude.degrees),  xycoords=ccrs.PlateCarree()._as_mpl_transform(ax1), ha='right', va='top')
        ax1.annotate(' T+15m ', xy=(subpoint_15.longitude.degrees,subpoint_15.latitude.degrees),  xycoords=ccrs.PlateCarree()._as_mpl_transform(ax1), ha='right', va='top')
    ax1.add_feature(Nightshade(ts.now().utc_datetime(), alpha=0.2))
    
##    # fall plot
##    ax2 = plt.subplot(3,1,3)
##    ax2.plot(t_long.astimezone(tz),to_ground.elevation.km)
##    ax2.set_ylabel('elevation (km)')
    
    plt.tight_layout()
    fig.canvas.draw() 
    fig.canvas.flush_events()
    plt.savefig('BBWU.png')
